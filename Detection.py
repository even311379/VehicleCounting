...
def get_distance(P):
    ...
    PL1 = np.array([60,120])
    PL2 = np.array([180,60])
    PR1 = np.array([395,127.5])
    PR2 = np.array([160,245])
...
def run(video,output = False,f_skip_rate = 5):
    ...
    oLoR = {'L':0,'R':0} # output
    def DelLogic(veh):
        if len(veh.pos_history) >= 2 and np.linalg.norm(veh.pos_history[0]-veh.pos_history[-1]) >= 60:
            if veh.LorR == 'L':
                oLoR['L'] += 1
                print('L += 1')
            elif veh.LorR == 'R':
    ...
    if output:
        writer = skvideo.io.FFmpegWriter("output/0623/Detected_{}.mp4".format(video.replace('/','_')[:-4]))
        f_id = 0
...
if __name__ == '__main__':
    videos = ['20180623/'+f for f in os.listdir('20180623/') if int(f[8:10]) <=18 and int(f[8:10])>=5]
    ...
    pd.DataFrame(outdict).to_excel('output/summary.xlsx')